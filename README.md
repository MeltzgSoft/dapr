...
1. **Manage libraries** — create named libraries, each a set of
   ...
   (device unplugged, share unreachable) are greyed out. Devices are re-probed
   in the background, so reconnecting one enables its library again without
   restarting Dapr; backend connections stay open only while Dapr is scanning,
   syncing, browsing, or performing one of those short probes.
